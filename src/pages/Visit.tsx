...
const quickFacts = [
  ...
  { icon: <FaClock className="text-blue-500 w-6 h-6" />, label: "Average Tour Duration", value: "60min" },
    ...
    <section className="bg-gray-200 text-gray-800 py-16 px-6 md:px-20">
      <div className="max-w-6xl mx-auto flex flex-col md:flex-row items-start md:items-center justify-between gap-10">
        ...
        <div className="md:w-1/2 bg-white border border-gray-200 rounded-3xl p-8 shadow-md">
          <h4 className="text-2xl text-blue-600 font-bold mb-6">Quick Facts</h4>
          <div className="flex flex-col gap-4">
            {quickFacts.map((fact, index) => (
              <div
                key={index}
                className="flex items-center gap-4 p-4 rounded-2xl border border-gray-200 hover:shadow-lg hover:scale-105 transition-transform duration-300 bg-white"
              >
                ...
                <div className="flex flex-col">
                  ...
                  <span className="text-blue-600 font-bold text-lg">{fact.value}</span>
                </div>
              </div>
            ))}
          </div>
